			...
			if($action == 'dashboard'){
				...
				if(count($get_po_listing) > 0){
				  ...
				  foreach($get_po_listing as $listingdetail){
						//if($i >= $startrec && $i < $totallength){
							...
							$color_code_html = '<select id="colorcode" name="colorcode" style="display:none;" ng-model="colorcode" ng-change="updateColorCode()">';
							...
							if($priority_status == 'low'){
								$colorval = 'yellow';
							} else if($priority_status == 'medium') {
								$colorval = 'orange';
							} else if($priority_status == 'high') {
								$colorval = 'red';
							} else {
								$colorval = 'green';
							}
				...
				if(count($get_all_vendors) > 0){
				  ...
				  foreach($get_all_vendors as $userinfo){
						...
						{
							$popovercontent = "Please click send email button to send invitation";
							...
							$popovercontent = "Email sent on ".date('F jS \a\t H:i',strtotime($userinfo->sent_date));
							...
							$popovercontent = "Invitation accepted on".date('F jS \a\t H:i',strtotime($userinfo->sent_date));
				...
				if($get_requester_info != 2){
					...
					$result['requestername'] =$requester_name;
				} else {
					$result['status_code']=201;
				}
			}  else if($action == 'updatepriority'){
				$update_priority = PoDetail::update_po_priority($poid,$priority);
