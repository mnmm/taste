<?php
	class PoDetail extends Eloquent{
		...
		public static function insert_po_details($po_no,$po_date,$vendor_id,$vendor_name,$city,$state,$country,$zip,$totalamount,$pickup_time,$status,$paid_status,$approved_by,$priority){
			...
			$po_id = DB::table('taste_po')->insertGetId($insertPoDetailAr);
			return $po_id;
		...
		public static function insert_po_item_details($po_no,$item_id,$name,$price,$quantity){
			...
			$poDescriptionAr = array('po_no' => $po_no,'product_id' => $item_id,'product_title'=>$name,'price'=>$price,'quantity'=>$quantity); 
			$po_desc_id = DB::table('taste_po_description')->insertGetId($poDescriptionAr);
			return $po_desc_id;
		...
		public static function create_vendor_account($email,$password){
			...
			if(isset($check_vendor_account->id) && $check_vendor_account->id != ''){
				...
				if(isset($get_vendor_name_info->update_account_email_id)){
					if(strpos($get_vendor_name_info->vendor_name,' ') !== false) {
						...
						$register->lastname = $get_vendor_name_info->vendor_name;
					} 
				}  else {
					...
					if(strpos($vendor_name_info->vendor_name,' ') !== false) {
						$extract_name = explode(' ',$vendor_name_info->vendor_name);
						$register->firstname = $extract_name[0];
						$register->lastname = $extract_name[1];
					} else {
						$register->firstname = $get_vendor_name_info->vendor_name;
	   ...
	   //function to update payment status
		...
		public static function get_transfer_option_vendor($poid){
			
			$get_vendor_info =  DB::table('taste_po')->where('po_no','=',$poid)->first();
			if($get_vendor_info->update_account_email_id != 0){
				$get_vendor_email = DB::table('updated_vendors_emails')->where('id',$get_vendor_info->update_account_email_id)->first();
				$vendor_email  = $get_vendor_email->email;
			} else {
				$vendor_email  = $get_vendor_info->vendor_email;
